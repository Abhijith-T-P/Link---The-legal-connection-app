    ...
    useEffect(() => {
        const fetchPermitRequests = async () => {
            try {
                ...
                const filteredPermitRequests = permitRequestsList.filter(permit => permit.UserID === uid);
                ...
                const joinData  = filteredPermitRequests.map(permit => ({
                   ...permit,
                    permitType: permitTypeList.find(cat => cat.id === permit.permitType),
            ...
            {loading ? (
                ...
                permitRequests.map((permit) => (
                    <Card key={permit.id} sx={{ marginTop: 2 }}>
                        <CardContent>
                        <Typography variant="h6">Permit Type: {permit.permitType.permit}</Typography>
                            <Typography>Applicant Name: {permit.applicantName}</Typography>
                            <Typography>Applicant Address: {permit.applicantAddress}</Typography>
                            <Typography>Reason: {permit.reason}</Typography>
                            <Typography>Request Date: {new Date(permit.timestamp.seconds * 1000).toLocaleString()}</Typography>
                            <Typography
    variant="body1"
    sx={{
        color: permit.pStatus === 1 ? 'green' : permit.pStatus === 2 ? 'red' : 'inherit',
    }}
>
    Status: {permit.pStatus ? (permit.pStatus === 1 ? "Accepted" : "Rejected") : "Pending"}
</Typography>
                        </CardContent>
